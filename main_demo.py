...
def do_many_things(data, flag=True, x=10, y=20, z=30):
    ...
    if (x > 0 and y > 0 and z > 0 and x + y + z > 50 and x * y * z > 5000
        and (x - y) != 0 and (y - z) != 0 and (x - z) != 0
        and str(x).isdigit() and str(y).isdigit() and str(z).isdigit()):
        print("complex condition met for values that honestly should probably be validated somewhere else in smaller helper functions")
